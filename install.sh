...
CONF
  ...
  sudo nginx -t && sudo systemctl reload nginx
  sudo mysql -u root -p$MYSQL_ROOT_PASS -e "CREATE DATABASE IF NOT EXISTS \`\$DB_NAME\`; GRANT ALL PRIVILEGES ON \`\$DB_NAME\`.* TO '\$DB_NAME'@'localhost' IDENTIFIED BY '\$DB_PASS';"
